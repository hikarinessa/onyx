...
- **The calendar moves its "today" highlight when the day changes** (#104). It relied on a timeout set for midnight, which fires late after the Mac sleeps; it now checks every minute and whenever the window comes back
- **One rule decides where a wikilink points, everywhere** (#70). Backlinks, clicking a link, dimming broken links and rewriting links on rename all follow the same resolution: a `[[folder/note]]` path from a registered root, then a note of that name in the linking note's own folder, then the first path alphabetically. The backlinks panel used to match link text exactly, so `[[idea]]` or `[[Notes/Consent]]` never appeared as backlinks and a duplicate name showed under both notes. A link written before its note existed now connects when the note appears, and links move when a note they name is renamed or deleted. Link text is never treated as a pattern, so `[[a_b/Note]]` can no longer match `aXb/Note.md`. The index records its schema version and commits each file in one transaction
- **Escape in the note title cancels the rename.** It reset the text and then saved the edited name anyway
...
- Footnotes inside table cells and embedded notes (`![[note]]`) still show as raw `[^label]` — tables and embeds render through their own paths
