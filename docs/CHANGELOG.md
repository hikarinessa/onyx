...
- **Wikilinks resolve the same way everywhere, and more of them resolve** (#70). Backlinks and click-to-follow now agree on which note a duplicate name means (the first by path). Links written before their target existed now connect when it appears, even when the case differs or the link names a folder path like `[[Notes/Consent]]`; after a full reindex those used to stay broken until the linking note was edited. The index also commits each file in one transaction and records its schema version for future upgrades
