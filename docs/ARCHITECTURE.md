...
8. **Click semantics** — Click replaces current tab, Cmd+click opens in new tab. Uniform across all navigation surfaces (sidebar, wikilinks, calendar, backlinks, recent docs, bookmarks, Quick Open). Matches browser link behavior. In-editor link clicks (wikilinks, URLs) use `mousedown` (not `click`) in preview mode to fire before CM6 moves cursor and removes focus-line decorations. **All in-editor click dispatch must use `view.posAtCoords()` + regex matching against the document text** — never DOM class names or `textContent`, because CM6's syntax highlighting splits text into opaque `<span>` elements that don't reliably expose decoration classes. One handler in `wikilinks.ts` owns the full dispatch chain: wikilink → markdown link → bare URL → pass through.
